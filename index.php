				...
				if($_POST['password'] != $_POST['confirm']){
					...
					if (!($conn->query($sql) === TRUE)) {
					   $register_error = "No Connection with database now try at another time";
						...
						$message = '<form action = "http://localhost/whatfitfat/confirm.php" method = "POST">
...
<section id="register" 
	...
	<img src="images/logo.jpg">
	<form method="POST" action="<?php echo $_SERVER['PHP_SELF'] ?>" autocomplete="on">
		<label style="color: <?php if($register_error != NULL){echo "red";}
			elseif ($register_succeed != NULL) { echo "green";  } ?>" >
			<?php 
			if($register_error != NULL){echo $register_error;}
